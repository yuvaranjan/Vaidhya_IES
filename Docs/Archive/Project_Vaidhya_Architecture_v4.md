# Project Vaidhya: AI-Powered Offline Telemedicine Architecture (v4)

*Supersedes v3. This version adds two Open Innovation-track additions: an explainability layer on the Agentic Specialist consult / diagnostic summary (5.1), and a lightweight urgency/severity indicator derived from the existing vitals-driven branching logic (1.2.2). Both are extensions of existing Vaidhya components, not new subsystems — see 5.5 and 1.7.*
...
### 1.7 Urgency Indicator (Lightweight Severity Signal)
...
- **Not an ESI clone.** This is a lightweight extension of logic Vaidhya already has, not a new triage subsystem. It reuses the same vitals-driven branching rules from 1.2.2 (low SpO2, fever pattern, elevated BP, abnormal pulse, concerning exam findings like RLQ tenderness) that already flag "higher-urgency" during question branching — this section just makes that internal signal visible as output.
- **Mechanism:** Each vitals/exam-finding rule in 1.2.2 that currently flags "higher-urgency" internally is mapped to a simple three-tier label — **Routine / Elevated / Urgent** — using the same rules table already planned for 1.2.2. No new model, scoring formula, or ML component is introduced; it's a presentation-layer read of existing branching flags.
- **Where it shows:**
  - On the diagnostic summary itself, as a short label with the one or two vitals/findings that drove it (e.g., "Elevated — low SpO2 (91%), reported breathlessness").
  - On the doctor's waiting-room queue (2.1), as a visual tag next to each waiting patient, so doctors can prioritize which report to open first — the doctor still chooses who to pick up next (doctor-initiated pickup is unchanged from 2.1); this only informs that choice, it doesn't auto-assign or reorder the queue automatically.
- **Explicit scope boundary:** This does not replace clinical judgment, does not auto-triage, and does not gate doctor access to any patient. It's an at-a-glance signal, consistent with the platform's assistive (not autonomous) design posture throughout.
...
- **Explainability:** every advisory opinion returned by this module includes a confidence score and evidence trace — see 5.5 for the full mechanism. This is additive to the interaction model above; the doctor-triggered, doctor-in-control flow is unchanged.
...
#### 5.5 Explainability Layer (Specialist Consult & Diagnostic Summary)
- **Purpose:** Give the doctor a visible reason to trust (or question) an AI-generated opinion, rather than a bare recommendation. This applies primarily to the Agentic Specialist advisory (5.1), and secondarily to the urgency indicator (1.7).
- **Scope decision:** this is a text-based confidence + evidence trace, not the full saliency-map/attention-heatmap/counterfactual explainability stack sometimes seen in imaging-heavy clinical AI systems. Vaidhya's inputs are conversational/vitals-based rather than deep-imaging-based (see 4, Visual Symptom Capture — a single still image described in text, not a segmented/classified scan), so a visual saliency layer wouldn't have meaningful pixel-level attention to visualize. The scope below is intentionally matched to what the platform actually processes.
- **What's returned alongside every specialist advisory opinion:**
  - **Confidence score:** a self-reported confidence level (e.g., High / Medium / Low) from the specialist LLM call, prompted for explicitly as part of the response format.
  - **Evidence trace:** a short list of which specific inputs from the diagnostic report (symptoms, vitals, transcript excerpts, exam findings) the opinion was primarily based on — surfaced as plain-language bullet points, not a raw attention map.
  - **Decision trace:** one or two lines on the reasoning path (e.g., "Low SpO2 + reported breathlessness on exertion → prioritized respiratory differential over GI causes"), so the doctor can see the *why*, not just the *what*.
- **Mechanism:** achieved through prompt design on the existing specialist LLM call from 5.1 — the system prompt is extended to require confidence, evidence trace, and decision trace as structured fields in the response, rather than free-form advisory text. No separate explainability model or pipeline is introduced.
- **Applied to 1.7:** the urgency indicator's "one or two vitals/findings that drove it" display is itself a minimal evidence trace, using the same principle — every urgency label is shown with its driving factors, never as a bare tag.
- **Open detail to finalize later:** exact structured-output schema (JSON fields vs. formatted text) for the confidence/evidence/decision trace, to be settled alongside the 5.1 prompt design.
...
- Agentic specialist Phase 1: exact prompt/context design per specialty (case data scope is now settled — full diagnostic report), now extended to also require confidence/evidence/decision-trace fields per 5.5.
...
- Explainability layer (5.5): exact structured-output schema for confidence/evidence/decision trace fields, to be settled alongside the 5.1 prompt design.
- Urgency indicator (1.7): final wording/thresholds for the Routine/Elevated/Urgent tiers, to be settled alongside the 1.2.2 rules table as it's built out.
